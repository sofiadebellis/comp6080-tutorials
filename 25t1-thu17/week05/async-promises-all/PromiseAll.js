// TODO
function question1() {
  const microsoftRepo = getCompanyRepos("microsoft");
  const googleRepo = getCompanyRepos("google");
  const canvaRepo = getCompanyRepos("canva");

  Promise.all([microsoftRepo, googleRepo, canvaRepo]).then((companies) => {
...
async function question2() {
  ...
  try {
    const companies = await Promise.all([microsoftRepo, googleRepo, canvaRepo]);
    companies.forEach((company) => {
      company.forEach((repo) => console.log(repo));
    });
  } catch (error) {
