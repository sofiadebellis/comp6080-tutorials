...
describe('Tictactoe', () => {
	...
	it('Changes board state when it is a tie', () => {
		cy.get('#0').click();
		cy.get('#1').click();
		cy.get('#3').click();
		cy.get('#4').click();
		...
		cy.get('#result').then((result) => {
			expect(result.text()).to.equal('Win player : ');
		});

		// Tie result should should all have white background color
		for (let index = 0; index < 9; ++index) {
			const cell = cy.get(`#${index}`);
			if ([0, 2, 3, 7, 8].includes(index)) {
				cell.should('have.css', 'background-color', 'rgba(0, 0, 0, 0)');
				cell.then((cell) => {
					expect(cell.text()).to.contain('X');
				});
